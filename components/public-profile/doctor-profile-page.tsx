  ...
  if (apiTimeSlots.length > 0) {
    ...
    for (const slot of daySlots) {
      ...
      while (currentMinutes + duration <= endMinutes) {
        const currentTimeStr = `${Math.floor(currentMinutes / 60)
          .toString()
          .padStart(
            2,
            "0",
...
const DOCTOR = {
  ...
  address:
    "1234 Medical Plaza, Suite 200, Connaught Place, New Delhi, Delhi 110001",
...
const SERVICES: ServiceOption[] = [
  {
    ...
    price: 500,
    enabled: true,
    description: "One-on-one video consultation",
  },
  {
    id: "chat",
    type: "service",
  ...
  },
  {
    ...
    price: 1500,
    enabled: true,
    description: "In-person visit at your residence",
  },
  {
    id: "emergency",
    type: "service",
    ...
    price: 2000,
    enabled: true,
    description: "Urgent consultations (priority)",
  },
  {
    id: "subscription",
    type: "service",
  ...
  },
  {
    ...
    price: -1,
    enabled: true,
    description: "Follow-up for returning patients (reduced rates)",
  },
];
...
type ViewMode = "main" | "booking" | "followup" | "success" | "verify";

export function DoctorProfilePage({ doctorId }: DoctorProfilePageProps) {
  const router = useRouter();
  const { user } = useAuth();
  const { setPendingBooking, pendingBooking, clearPendingBooking } =
    useBooking();
    ...
    const fetchTimeSlots = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/doctor/${doctorId}/public-timeslots?date=${today}`,
        );
        const result = await response.json();
  ...
  useEffect(() => {
    const fetchAppointmentsForDay = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/doctor/${doctorId}/public-timeslots?date=${dateStr}`,
        );
        const result = await response.json();
  ...
  const availableSlots = useMemo(
    () => getSlotsForDate(selectedDay, timeSlots, appointments),
    [selectedDay, timeSlots, appointments],
  );
                ...
                <div ref={timeSectionRef}>
                  ...
                  <div className="grid grid-cols-7 gap-1.5 mb-4">
                    {days.map((day) => {
                      ...
                      const dayAvailable = getSlotCountForDate(
                        day,
                        timeSlots,
                        appointments,
                      );
                      return (
                        ...
                        >
                          ...
                          {isToday && (
                            <div
                              className={`size-1 rounded-full ${isSelected ? "bg-primary-foreground" : "bg-primary group-hover:bg-primary-foreground"}`}
                            />
                          )}
                  ...
                  <CardContent className="px-5 py-0">
                    <div className="flex flex-col gap-2.5">
                      ...
                      <InfoRow
                        ...
                        label="Patients"
                        value={currentDoctor.patientsServed}
                      />
